...
void Data::set_feature_vector(std::vector<double> *vect) {
  feature_vector_ = vect;
}
void Data::append_to_feature_vector(const double val) const {
  feature_vector_->emplace_back(val);
}
void Data::set_label(const uint8_t label) { label_ = label; }
std::size_t Data::get_feature_vector_size() const { return feature_vector_->size(); }
