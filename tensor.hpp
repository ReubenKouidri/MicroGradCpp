...
template<typename T>
class BaseTensor {
  // template<typename C> friend class Tensor;
  template<typename C>
...
class Tensor {
  template<typename C>
  friend std::ostream& operator<<(std::ostream& os, const Tensor<C>& val) {
    os << "Tensor(" << val.get_data() << ", " << val.get_grad() << ")";
    return os;
  }
  ...
  // 'right-op' overloads
  template<class C>
  friend Tensor<C> operator+(C num, const Tensor<C>& val) { return val + num; }
