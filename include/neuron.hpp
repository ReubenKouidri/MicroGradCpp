...
class Neuron final : public Module<T> {
 ...
 public:
  Neuron(std::size_t, std::size_t, const UnaryOp&);
  Neuron(const Neuron&);
  ...
  Value<T> operator()(const std::vector<Value<T>>&) const;
  Value<T> operator()(const std::vector<T>&) const;
  [[nodiscard]] T predict(const std::vector<T>&) const;
};
